...
contains
    ...
    real*8 function kah_1to(x,y,h)
        real*8, intent(in) :: x, y, h
        kah_1to = h * func(x,y)
    end function

    real*8 function kah_2to(x,y,h)
        real*8, intent(in) :: x, y, h
        kah_2to = h * func(x + (h/2.0d0), y + (kah_1to(x,y,h)/2.0d0))
    end function
    
    real*8 function kah_3to(x,y,h)
        real*8, intent(in) :: x, y, h
        kah_3to = h * func(x + h, y - kah_1to(x,y,h) + 2.0d0 * kah_2to(x,y,h))
    end function

    real*8 function rk_to(x,y,h)
        real*8, intent(in) :: x, y, h
        rk_to = y + (kah_1to(x,y,h) + 4.d0 *kah_2to(x,y,h) + kah_3to(x,y,h)) / 6.0d0
    end function
end module
...
program edos
    ...
    do i = 0, i_total - 1
        ...
        yvals(i+1) = rk_to(x_atual, yvals(i), h)
